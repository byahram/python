...
-- select문으로 mem_id, mem_name, addr 을 조회하면 3개의 컬럼을 가진 테이블과 유사한 형태가 조회됨
...
as
	select문;
-- 일반적으로 뷰의 이름만 보고도 뷰인지 알아볼 수 있도록 이름 앞에 v_ 를 붙이는 것이 일반적
...
-- 만약 위의 쿼리를 자주 사용해야 한다면 매번 쿼리를 입력하기 힘듦
-- 이 때, 이 sql을 뷰로 생성해두고 사용자들이 뷰에 접근하도록 한다면 복잡한 sql을 입력할 필요가 없어짐
...
as
	...
	from buy b
		inner join member m
		on b.mem_id = m.mem_id;
-- v_memberbuy를 테이블처럼 접근할 수도 있고 where절도 사용할 수 있음
select * from v_memberbuy;
select * from v_memberbuy where mem_name = '블랙핑크';

-- 뷰의 실제 작동
-- 뷰의 실제 생성, 수정, 삭제
-- 뷰를 생성하면서 뷰에서 사용될 열이름을 테이블과 다르게 지정할 수도 있음(별칭을 사용해서)
-- 별칭은 열 이름 뒤에 작은 따옴표 또는 큰 따옴표로 묶어주고, as 를 붙여 코드를 명확하게 보여줌
...
-- drop view와 create view 를 연속해서 사용한 효과를 가짐

describe v_viewtest2;
-- describe 로 뷰의 정보를 확인할 수 있음
...
-- 에러 발생
-- v_member 뷰가 참조하는 member 테이블의 열 중에서 mem_number 열은 not null 열이라서
-- 값이 반드시 입력되어야 하지만
-- 현재의 v_member 에서는 mem_number 열을 참조하고 있지 않으므로 값을 입력할 방법이 없음

-- 만약 v_member 뷰를 통해서 member 테이블에 값을 입력하고 싶다면
-- v_member 에 mem_number를 포함하도록 뷰를 재정의하거나
-- member에서 mem_number 열의 속성을 null로 바꾸거나 기본값을 지정해야 함
...
-- height가 167 이상인 회원만 조회되었음

-- v_height167 뷰에서 height가 167 미만인 데이터를 삭제하기
delete from v_height167 where height < 167;
select * from member;
-- v_height167뷰에는 167미만의 데이터가 없으므로 삭제될 데이터가 없음

-- 뷰를 통한 데이터 입력
-- v_height167뷰에 height 가 167미만인 데이터 입력하기
insert into v_height167
values ('TRA', '티아라', 6, '서울', null, null, 159, '2005-01-01');
select * from v_height167;
select * from member;
-- 데이터 입력은 되었지만 v_height167뷰는 height167 이상만 조회가 되므로
-- 마지막으로 입력한 티아라는 보이지 않음

-- height가 167 이상인 뷰에 height 가 159인 데이터를 입력하는 것은 바람직하지 않음
-- 논리적으로 바람직하려면 height가 167 이상인 뷰에 height가 167 이상인 데이터만 입력되는 것이 알맞음
...
-- 이제 height가 167 미만인 데이터는 입력되지 않고 167이상의 데이터만 입력됨
...
-- 복합 뷰는 읽기 전용임. 복합 뷰를 통해 테이블에 데이터를 입력/수정/삭제할 수 없음
